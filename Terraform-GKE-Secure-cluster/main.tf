...
resource "google_compute_router_nat" "mist_nat" {
 ...
 # Create the GKE control plan for the cluster with enable monitoring and logging for Kubernetes components and enable managed Prometheus for the cluster
...
# Create a service account first for the worker nodes to give access to other GCP services such as 
# Cloud monitoring and Cloud Logging if we want to enable managed Prometheus in the cluster
...
# The K8s node pool will provision a node pool with auto scaling enabled as minimum 0 and max 2 nodes per zone, and using instance type e2-medium
